locals {
  ...
  # LLM provider switch (ADR-006): one provider serves every LLM task. Flip this
  # to "claude" to switch providers everywhere; "deepseek" requires the
  # deepseek-api-key-prod secret version to exist before apply (infra/README.md).
  ...
  # matters for direct-to-Cloud-Run calls.
...
module "extract_worker" {
  ...
  max_instances      = 5 # P7-IN-2: prod extract-worker cap
  concurrency        = 4
  allow_push_invoker = true
  push_auth_sa_email = google_service_account.push_auth.email
  env_vars = merge(local.svc_db_env, {
  ...
  # exist before apply (claude-api-key-prod always; deepseek-api-key-prod only
  # when local.llm_provider = "deepseek" — ADR-006), else the revision fails.
  secret_env = merge(
...
module "database" {
  ...
  env          = local.env # -> availability_type REGIONAL (module sizes on env)
  region       = var.region
  db_tier      = var.db_tier
  ipv4_enabled = true # connector + IAM auth only; private IP is Tier 1
  iam_user_emails = [
    module.api.sa_email,
    module.scrape_worker.sa_email,
    module.extract_worker.sa_email,
  ]
  deletion_protection = true # P7-IN-2: prod
...
module "blobstore" {
  ...
  force_destroy          = false # prod: never force-destroy the raw bucket
...
# --- Scheduler (P7-IN-4: live in prod) ---------------------------------------
# Global scrape.tick cron, Europe/Paris. Unlike dev this is NOT paused — prod is
# where the scheduled scrape actually runs.
module "scheduler" {
